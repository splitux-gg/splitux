...
pub fn launch_cmds(
    ...
    for (i, instance) in instances.iter().enumerate() {
        ...
        // Force SDL to use Linux evdev/input driver directly (bypasses Wayland focus)
        cmd.env("SDL_JOYSTICK_LINUX_EVDEV", "1");
        // Disable udev for joystick detection - use direct evdev enumeration
        cmd.env("SDL_JOYSTICK_LINUX_CLASSIC", "1");
        // Disable Steam Input integration which can interfere
        cmd.env("SDL_GAMECONTROLLER_USE_BUTTON_LABELS", "1");
        // Force input driver to use evdev directly
        cmd.env("SDL_VIDEODRIVER", "x11");  // Use X11 in nested Xwayland for input

        // Tell SDL which specific device(s) this instance should use
        // This helps when multiple controllers exist - SDL will only see the specified device
        let gamepad_paths: Vec<String> = instance.devices.iter()
            .filter_map(|&d| {
        ...
        if !gamepad_paths.is_empty() {
            // SDL_JOYSTICK_DEVICE takes comma-separated list of device paths
            cmd.env("SDL_JOYSTICK_DEVICE", gamepad_paths.join(","));
        ...
        // Collect keyboard/mouse devices for gamescope-kbm's libinput-hold-dev
        // (gamepads are handled separately via evdev/SDL, not libinput)
        if cfg.kbm_support {
            ...
            if !kbm_devices.is_empty() {
                cmd.arg(format!("--libinput-hold-dev={}", kbm_devices.trim_end_matches(',')));
            }
        }
        cmd.arg("--");
        ...
        // This prevents issues with multiple controllers being mapped incorrectly
        // through the legacy joystick interface
        let js_devices = glob_js_devices();
        println!("[splitux] Instance {}: Blocking {} js devices: {:?}", i, js_devices.len(), js_devices);
        for js_path in &js_devices {
            cmd.args(["--bind", "/dev/null", js_path]);
        }

        // Mask out any gamepads that aren't this player's (evdev devices)
        // This instance has devices: {:?}
        println!("[splitux] Instance {}: Assigned device indices: {:?}", i, instance.devices);
        for (d, dev) in input_devices.iter().enumerate() {
            if !dev.enabled
                || (!instance.devices.contains(&d) && dev.device_type == DeviceType::Gamepad)
            {
                println!("[splitux] Instance {}: Blocking {} (type={:?}, in_devices={})",
                    i, dev.path, dev.device_type, instance.devices.contains(&d));
