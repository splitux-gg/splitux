...
fn expand_dot_notation(value: serde_yaml::Value) -> serde_yaml::Value {
    ...
    for (key, val) in map {
        let Value::String(key_str) = &key else {
            ...
            result.insert(key, expand_dot_notation(val));
            continue;
        };

        if key_str.contains('.') {
            // Split "goldberg.disable_networking" into ["goldberg", "disable_networking"]
            let parts: Vec<&str> = key_str.split('.').collect();
            insert_nested(&mut result, &parts, val);
        } else {
            // No dot, keep as-is but recursively expand nested values
            result.insert(key, expand_dot_notation(val));
        }
    }
